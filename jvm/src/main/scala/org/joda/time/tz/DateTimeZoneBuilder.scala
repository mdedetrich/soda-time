...
object DateTimeZoneBuilder {
  ...
  private class RuleSet() {
    ...
    private var iStandardOffset: Int = _
    private var iRules: ArrayList[Rule] = new ArrayList[Rule](10)
    ...
    def firstTransition(firstMillis: Long): Transition = {
      ...
      val copy = new ArrayList[Rule](iRules)
    ...
    def buildTailZone(id: String): DSTZone = {
      if (iRules.size == 2) {
        val startRule = iRules.get(0)
        val endRule = iRules.get(1)
  ...
  object PrecalculatedZone {
    ...
    def readFrom(in: DataInput, id: String): PrecalculatedZone = {
      ...
      val pool = Array.ofDim[String](poolSize)
      for (i <- 0 until poolSize) {
        pool(i) = in.readUTF()
      }
      val size = in.readInt()
      val transitions = Array.ofDim[Long](size)
      val wallOffsets = Array.ofDim[Int](size)
      val standardOffsets = Array.ofDim[Int](size)
      val nameKeys = Array.ofDim[String](size)
      for (i <- 0 until size) {
    ...
    def create(id: String,
      ...
      val trans = Array.ofDim[Long](size)
      val wallOffsets = Array.ofDim[Int](size)
      val standardOffsets = Array.ofDim[Int](size)
      val nameKeys = Array.ofDim[String](size)
      var last: Transition = null
      for (i <- 0 until size) {
        val tr = transitions.get(i)
      ...
      var zoneNameData = Array.ofDim[String](5)
  ...
  class PrecalculatedZone private (id: String,
                                   private val iTransitions: Array[Long],
                                   private val iWallOffsets: Array[Int],
                                   private val iStandardOffsets: Array[Int],
                                   private val iNameKeys: Array[String],
                                   private val iTailZone: DSTZone) extends DateTimeZone(id) {

    def getNameKey(instant: Long): String = {
      val transitions = iTransitions
      var i = Arrays.binarySearch(transitions, instant)
      if (i >= 0) {
        return iNameKeys(i)
      }
      i = ~i
    ...
    def nextTransition(instant: Long): Long = {
      var _instant: Long = instant
      val transitions = iTransitions
      var i = Arrays.binarySearch(transitions, _instant)
      i = if (i >= 0) (i + 1) else ~i
      if (i < transitions.length) {
    ...
    override def equals(obj: Any): Boolean = {
      ...
      if (obj.isInstanceOf[PrecalculatedZone]) {
        ...
        return getID == other.getID && Arrays.equals(iTransitions, other.iTransitions) &&
          Arrays.equals(iNameKeys.map(_.asInstanceOf[AnyRef]), other.iNameKeys.map(_.asInstanceOf[AnyRef])) &&
    ...
    def writeTo(out: DataOutput) {
      ...
      val poolSet = new HashSet[String]()
      for (i <- 0 until size) {
...
class DateTimeZoneBuilder {
  ...
  private val iRuleSets = new ArrayList[RuleSet](10)
  ...
  def addCutover(year: Int,
    ...
    if (iRuleSets.size > 0) {
      ...
      val lastRuleSet = iRuleSets.get(iRuleSets.size - 1)
  ...
  private def addTransition(transitions: ArrayList[Transition], tr: Transition): Boolean = {
    val size = transitions.size
    if (size == 0) {
      transitions.add(tr)
      return true
    }
    val last = transitions.get(size - 1)
    if (!tr.isTransitionFrom(last)) {
      return false
    }
    var offsetForLast = 0
