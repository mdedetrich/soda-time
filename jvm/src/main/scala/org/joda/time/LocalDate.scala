...
import org.joda.time.convert.{ConverterManager, PartialConverter}
...
object LocalDate {
  ...
  private val DATE_DURATION_TYPES = new HashSet[DurationFieldType]()
  ...
  def fromDateFields(date: Date): LocalDate = {
    ...
    new LocalDate(date.getYear + 1900, date.getMonth + 1, date.getDate)
  }

  @SerialVersionUID(-3193829732634L)
  class Property(@transient private var iInstant: LocalDate,
...
class LocalDate(instant: Long, private var chronology: Chronology)
    extends BaseLocal
  ...
  def this(instant: AnyRef, zone: DateTimeZone) {
    ...
    iLocalMillis =
      iChronology.getDateTimeMillis(values(0), values(1), values(2), 0)
  }
  ...
  def this(year: Int,
    ...
    val instant =
      _chronology.getDateTimeMillis(year, monthOfYear, dayOfMonth, 0)
  ...
  def toDate(): Date = {
    ...
    if (check.isBefore(this)) {
      ...
      val earlier = new Date(date.getTime - TimeZone.getDefault.getDSTSavings)
  ...
  def withLocalMillis(newMillis: Long): LocalDate = {
    ...
    if (_newMillis == getLocalMillis) this
    else new LocalDate(_newMillis, getChronology)
  }
