...
object DateTimeFormat {
  ...
  private def parseToken(pattern: String, indexRef: js.Array[Int]): String = {
  ...
  private def createFormatterForPattern(pattern: String): DateTimeFormatter = {
    ...
    var formatter = cPatternCache.get(pattern).orNull
    if (formatter == null) {
      val builder = new DateTimeFormatterBuilder()
      parsePatternTo(builder, pattern)
      formatter = builder.toFormatter()
      if (cPatternCache.size < PATTERN_CACHE_SIZE) {
        val oldFormatter = if (cPatternCache.get(pattern).isEmpty) {
          cPatternCache(pattern) = formatter
          formatter
        } else {
          formatter
        }
  ...
  private def createFormatterForStyleIndex(dateStyle: Int, timeStyle: Int): DateTimeFormatter = {
    ...
    var f = cStyleCache(index)
    if (f == null) {
      f = createDateTimeFormatter(dateStyle, timeStyle)
      if (cStyleCache(index) == null) {
        cStyleCache(index) = f
      } else {
        f = cStyleCache(index)
      }
