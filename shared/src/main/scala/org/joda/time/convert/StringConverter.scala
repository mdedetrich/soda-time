...
import org.joda.time.{Chronology, Period, ReadWritableInterval, ReadWritablePeriod, ReadablePartial}
...
class StringConverter extends AbstractConverter() with InstantConverter with PartialConverter with DurationConverter with PeriodConverter with IntervalConverter {
  ...
  override def getPartialValues(fieldSource: ReadablePartial,
    ...
    val millis = parser.withChronology(_chrono).parseMillis(`object`.asInstanceOf[String])
  ...
  def getDurationMillis(`object`: AnyRef): Long = {
    ...
    } else seconds = if (negative) str.substring(firstDigit, str.length).toLong else str.toLong
    if (negative) {
  ...
  def setInto(writableInterval: ReadWritableInterval, `object`: AnyRef, chrono: Chronology) {
    ...
    if (c == 'P' || c == 'p') {
      ...
      period = periodParser.withParseType(getPeriodType(rightStr))
