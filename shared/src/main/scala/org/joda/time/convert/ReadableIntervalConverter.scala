...
class ReadableIntervalConverter protected ()
  ...
  def setInto(writablePeriod: ReadWritablePeriod,
    ...
    _chrono =
      if (_chrono != null) _chrono
      else DateTimeUtils.getIntervalChronology(interval)
  ...
  override def isReadableInterval(`object`: AnyRef,
                                  chrono: Chronology): Boolean = true
