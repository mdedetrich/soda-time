...
abstract class BaseDateTime(instant: Long, chronology: Chronology)
  ...
  def this(instant: AnyRef, chronology: Chronology) {
    ...
    iMillis = checkInstant(converter.getInstantMillis(instant, chronology),
                           iChronology)
  }
  ...
  def this(year: Int,
    ...
    val instant = iChronology.getDateTimeMillis(year,
                                                monthOfYear,
                                                dayOfMonth,
                                                hourOfDay,
                                                minuteOfHour,
  ...
  protected def auxConstructor(instant: AnyRef): Unit = {
    val converter = ConverterManager.getInstance.getInstantConverter(instant)
    val chrono = checkChronology(
      converter.getChronology(instant, null: Chronology))
    iChronology = chrono
    iMillis = checkInstant(converter.getInstantMillis(instant, chrono), chrono)
  }
  ...
  protected def checkChronology(chronology: Chronology): Chronology =
    DateTimeUtils.getChronology(chronology)

  protected def checkInstant(instant: Long, chronology: Chronology): Long =
    instant
