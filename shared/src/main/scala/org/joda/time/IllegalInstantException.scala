...
object IllegalInstantException {
  ...
  def create(instantLocal: Long, zoneId: String) = new IllegalInstantException(createMessage(instantLocal,zoneId))
  

  private def createMessage(instantLocal: Long, zoneId: String): String = {
    val localDateTime = DateTimeFormat.forPattern("yyyy-MM-dd'T'HH:mm:ss.SSS")
...
class IllegalInstantException(message: String) extends IllegalArgumentException(message)